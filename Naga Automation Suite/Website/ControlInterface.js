...
function pickTask(taskItem)
{
    ...
    for (var i = 0; i < taskHolder.length; i++)
    {
        ...
        {
            document.getElementById("taskName").innerHTML = "<a href='#' title='Click to access modify page' onClick='javascript:modifyView(\"" + taskHolder[i].name + "\", \"task\")'>" + taskHolder[i].name + "</a>";
            document.getElementById("taskDevice").innerHTML = taskHolder[i].device;
            
            var action = "";            
            if (taskHolder[i].actiontype == SEND_ENABLE)
                action += "On";
            
            else if (taskHolder[i].actiontype == SEND_DISABLE)
                action += "Off";
                
            else if (taskHolder[i].actiontype == SEND_WRITE)
                action += "Toggle";
